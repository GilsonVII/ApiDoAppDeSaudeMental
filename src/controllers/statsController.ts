...
export const handleGetUserStatus = async (req: Request, res: Response) => {
    try {
        const loggedInUserId = req.user?.id;
        if (!loggedInUserId) {
            return res.status(401).json({ error: 'Usuário não autenticado.' });
        ...
        const status = await statsBusiness.getUserStatus(loggedInUserId, patientId);
        return res.status(200).json(status);

    } catch (error: any) {
        Logger.error('Erro no controller de status do paciente:', error);
        if (error instanceof AppError) {
            return res.status(error.statusCode).json({ error: error.message });
        }
